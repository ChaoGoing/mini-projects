  ...
  _read_header_line(char) {
    ...
    if(char === 0x3A) {
      ...
      if(this._cache[1]) {
        ...
        const contentLenHeader = this._message.request.headers.filter(item => item.key === 'Content-Length')
        if(contentLenHeader && contentLenHeader[0] && contentLenHeader[0].value > 0) {
          this._cache = null
